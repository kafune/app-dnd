import type { Character, CharacterLogEntry, DiceRoll, HomebrewItem, HomebrewKind } from "./types";
...
export const api = {
  listCharacters: () => send<{ characters: Character[] }>("/api/characters", { cache: "no-store" }),
  ...
  listRolls: (limit = 50) => send<{ rolls: DiceRoll[] }>(`/api/rolls?limit=${limit}`, { cache: "no-store" }),
